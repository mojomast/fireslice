...
// CreateAndStartWithOptions starts a new VM using the fireslice-oriented
// create contract while preserving the existing launch flow.
func (m *Manager) CreateAndStartWithOptions(ctx context.Context, opts CreateOptions) error {
...
func validateCreateOptions(opts CreateOptions) error {
	switch {
	case opts.VMID <= 0:
		return fmt.Errorf("vm_id must be greater than zero")
	...
	case opts.MemoryMB <= 0:
		return fmt.Errorf("memory_mb must be greater than zero")
	case opts.DataDiskGB <= 0:
		return fmt.Errorf("data_disk_gb must be greater than zero")
	default:
		return nil
	}
}
...
func dataDiskSizeBytes(dataDiskGB int) (int64, error) {
	...
	return int64(dataDiskGB) * gib, nil
}

func (m *Manager) createDataDisk(ctx context.Context, vmID int64, dataDiskGB int) (string, error) {
	dataDiskPath := filepath.Join(m.dataDir, "disks", fmt.Sprintf("vm-%d-data.ext4", vmID))
	if _, err := os.Stat(dataDiskPath); err == nil {
		return dataDiskPath, nil
	} else if !os.IsNotExist(err) {
		return "", fmt.Errorf("stat data disk: %w", err)
	}

	sizeBytes, err := dataDiskSizeBytes(dataDiskGB)
	if err != nil {
		return "", err
	}

	if err := createEmptyExt4Func(ctx, dataDiskPath, sizeBytes); err != nil {
		return "", err
	}
...
func (m *Manager) Destroy(ctx context.Context, vmID int64) error {
	if err := m.DestroyResources(ctx, vmID); err != nil {
		return err
	}
...
// DestroyResources stops a VM (if running) and removes its runtime files
