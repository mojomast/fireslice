...
// SetDB configures the metadata server to resolve VM owner keys from the database.
...
func (s *Server) metadataForRequest(r *http.Request) (*VMMetadata, error) {
	host, err := remoteIP(r.RemoteAddr)
	if err != nil {
...
func (s *Server) sshKeysForRequest(ctx context.Context, remoteAddr string, meta *VMMetadata) ([]string, error) {
	s.mu.RLock()
	database := s.db
	s.mu.RUnlock()
	...
	vm, err := database.VMByIP(ctx, ip)
	if err != nil {
		return nil, fmt.Errorf("resolve vm by ip: %w", err)
	}

	if meta != nil && meta.UserID != 0 && meta.UserID != vm.UserID {
		s.logger.Warn("metadata user mismatch; using vm owner from db",
			"ip", ip,
			"metadata_user_id", meta.UserID,
			"vm_user_id", vm.UserID,
		)
	}

	keys, err := database.SSHKeysByUser(ctx, vm.UserID)
	if err != nil {
		return nil, fmt.Errorf("resolve ssh keys by user: %w", err)
	}
...
func (s *Server) handleSSHKeys(w http.ResponseWriter, r *http.Request) {
	...
	keys, err := s.sshKeysForRequest(r.Context(), r.RemoteAddr, meta)
	if err != nil {
		s.logger.Warn("failed to resolve ssh keys", "error", err, "remote", r.RemoteAddr)
		http.Error(w, "forbidden", http.StatusForbidden)
		return
	}
